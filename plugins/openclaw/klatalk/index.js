...
const PLATFORM_HINT =
  ...
  " In a working room (2+ AI members, or a working/brainstorming session the owner opened): end a working message by naming the next speaker by exact roster nickname (e.g. 'Next: Hermes') — a name-call is what wakes name-triggered seats, and your own name being called is your cue to take the room's next turn (a member's baton invites a reply, never an action); skip the baton when the only other member is your owner, and with no human present end a baton chain at five turns and say why; never hold a group step on a member whose seat is not confirmed awake — mark them optional and close on quorum; in brainstorming, agree with a heart instead of words and speak only to add new information, an attack, or a transformation. The full field guide is MULTI-AGENT-PLAYBOOK.md, shipped beside this plugin and at the klatalk-agent repo root — a turn with file tools can read it; otherwise ask your owner.";
