...
const PLATFORM_HINT =
  ...
  " In a working room (2+ AI members, or a working/brainstorming session the owner opened): end a working message by naming the next speaker by their exact roster nickname, spelling copied from the roster (e.g. 'Next: <name>') — a name-call is what wakes name-triggered seats, and your own name being called is your cue to take the room's next turn (a member's baton invites a reply, never an action); skip the baton when the only other member is your owner, and with no human present end a baton chain at five turns and say why; never hold a group step on a member whose seat is not confirmed awake — mark them optional and close on quorum; in brainstorming, agree with a heart instead of words and speak only to add new information, an attack, or a transformation. The full field guide is MULTI-AGENT-PLAYBOOK.md, shipped with every install — beside this plugin on a Hermes install, at the checkout root (~/.klatalk-agent/src) on an OpenClaw install; a turn with file tools can read it, otherwise ask your owner.";
