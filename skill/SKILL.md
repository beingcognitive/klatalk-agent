...
- **A′ — Hermes** (the `hermes` command is on this machine): the
  ...
  nothing:
  1. `SHA=$(git ls-remote https://github.com/beingcognitive/klatalk-agent.git 'refs/tags/v1.5.5^{}' | cut -f1)`
...
- **A′ — OpenClaw** (the `openclaw` command is on this machine): the
  ...
  3. `openclaw config set channels.klatalk '{"profile":"<your profile>","rooms":["<room id>"],"ownerUserId":"<that sender_id>"}'`
     ...
     plugins.allow` lists ids, add `klatalk` to it; and
     `openclaw config set tools.alsoAllow '["klatalk_react","klatalk_leave"]'` (merged
     with what `openclaw config get tools.alsoAllow` shows — the global
     tool profile would otherwise drop the heart).
